...
var utils = {
  ...
  pageBannerCheck: function(){
    ...
    if ($('.index-sections').children().first().hasClass('under-menu')) {
      /* // suresh.katiki - to make header on top
...
$(function() {
  ...
  var quickShop = {
    ...
    updateVariant: function(variant){
      $('.js-sale-banner, .js-sold-out, .js-current-price, .js-was-price').empty();

      if (globalQuickShopProduct != 'undefined'){
        var product = globalQuickShopProduct;

        $('.js-new-banner, .js-pre-order-banner').html('');
...
/*-----------------------------------------------
suresh.katiki - scrroll to home products
-------------------------------------------------*/
$(document).ready(function() {
	$("#scroll_to_products").click(function() {
		$('html, body').animate({
			scrollTop: $("#home-product-list").offset().top - 20
		}, 1000);
	});
});

/*-----------------------------------------------
suresh.katiki Topper page - for updating cover price
-------------------------------------------------*/
$(document).ready(function() {
	$('.single-option-selector[data-option="option2"]').on('change', function() {
        if($("#include_cover").length != 0)
        {
            size = $('.single-option-selector[data-option="option2"] option:selected').val();
            size = size.replace(/ *\([^)]*\) */g, "");

            if(size=='Twin') 		$('#include_cover option[value="1"]').text('COVER ($49 will be added at checkout)');
            if(size=='Twin XL') 	$('#include_cover option[value="1"]').text('COVER ($49 will be added at checkout)');
            if(size=='Full') 		$('#include_cover option[value="1"]').text('COVER ($59 will be added at checkout)');
            if(size=='Queen') 		$('#include_cover option[value="1"]').text('COVER ($59 will be added at checkout)');
            if(size=='King') 		$('#include_cover option[value="1"]').text('COVER ($69 will be added at checkout)');
            if(size=='Cal King')	$('#include_cover option[value="1"]').text('COVER ($69 will be added at checkout)');
        }
    });
});

/*------------------------------------------------
suresh.katiki Product page - Mattress details
-------------------------------------------------*/
$(document).ready(function() {
	
	$('.openDetails').on('click', function(e) {
    	$('.details-chart').toggleClass("details-chart details-chart-show details-chart details-chart-hide");// to hide/show mattrss details.
    });
	
	$('.faqs_title').on('click', function(e) {
    	$(this).next('.faqs_content').toggleClass("faqs_content faqs_content_show faqs_content faqs_content_hide");// to hide/show faqs details.
    });
		
	$('.faqs-question').on('click', function(e) {
    	$(this).next('.faqs-ans').toggleClass("faqs-ans faqs-ans-show faqs-ans faqs-ans-hide");// to hide/show faqs details.
    	$(this).toggleClass("faqs-question faqs-question active-question");// to hide/show faqs details.
    });
  
    $(document).on('change keyup keypress keydown paste input', '.faq-search-input', function() {
        var txt = $(this).val();
      	if(txt!='')
        {
			//$('h1.faqs-heading').hide();
        ...
        {
			$('h1.faqs-heading').show(); 	
			$('div.faqs-question').show();
			$('div.faqs-ans').show();
        }
    });
});
...
$(document).ready(function() {
	...
	if($_GET['variant'] != '' && $("#include_cover").length != 0)
    {
        size = $('.single-option-selector[data-option="option2"] option:selected').val();
        size = size.replace(/ *\([^)]*\) */g, "");
...
/*-----------------------------------------------
suresh.katiki - scrroll to warranty section
-------------------------------------------------*/
$(document).ready(function() {
	$(".warranty_mattress").click(function() {
		$('html, body').animate({
			scrollTop: $("#warranty_mattress").offset().top - 30
		}, 1000);
	});
	$(".warranty_topper").click(function() {
		$('html, body').animate({
			scrollTop: $("#warranty_topper").offset().top - 30
		}, 1000);
	});
	$(".warranty_pillow").click(function() {
		$('html, body').animate({
			scrollTop: $("#warranty_pillow").offset().top - 30
		}, 1000);
	});
	$(".warranty_foundation").click(function() {
		$('html, body').animate({
			scrollTop: $("#warranty_foundation").offset().top - 30
		}, 1000);
	});
});
...
/*-----------------------------------------------
suresh.katiki - to fix reviews issue when loading page
-------------------------------------------------*/
$(document).ready(function() {
    yotpo.initWidgets();
});